...
function initBlock(block, popup) {
	...
	// Initialize popup with first image
	const currentImageIndex = { value: 0 };
	initPopupActions(popup, imagesList, currentImageIndex);

	// Open popup when action button is clicked
	button.addEventListener('click', () => {
		togglePopup(true);
		updatePopupImage(imagesList, currentImageIndex.value, popup);
	});
...
function updatePopupImage(imagesList, index, popup) {
	...
	const imageClone = imagesList[index].cloneNode(true);
	const miniMapImageClone = imagesList[index].cloneNode(true);
...
function initPopupActions(popup, imagesList, currentImageIndex) {
	...
	prevButton.addEventListener('click', () => {
		...
		currentImageIndex.value = (currentImageIndex.value - 1 + imagesList.length) % imagesList.length;
		updatePopupImage(imagesList, currentImageIndex.value, popup);
	});
	...
	nextButton.addEventListener('click', () => {
		...
		currentImageIndex.value = (currentImageIndex.value + 1) % imagesList.length;
		updatePopupImage(imagesList, currentImageIndex.value, popup);
	});
