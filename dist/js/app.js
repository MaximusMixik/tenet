(() => {
    ...
    class ArticleNavigation {
        constructor() {
            ...
            this.navigationSectionsList = this.navigationLinksList.map((link => {
                const id = link.getAttribute("href")?.slice(1);
                return document.getElementById(id);
            })).filter(Boolean);
            if (this.navigationLinksList.length && this.navigationSectionsList.length) this.initObserver();
        }
        initObserver() {
...
*/    var _suppressOverwrites, _reverting, _context, _globalTimeline, _win, _coreInitted, _doc, _coreReady, _lastRenderedFrame, _quickTween, _tickerActive, _config = {
    ...
    Tween.version = Timeline.version = gsap_core_gsap.version = "3.13.0";
    _coreReady = 1;
    _windowExists() && _wake();
...
*/
    ...
    function getTargetElements(target) {
        ...
        return dist_toArray(elements).reduce((function(result, element) {
            return [].concat(_toConsumableArray(result), _toConsumableArray(dist_toArray(element).filter(dist_isNode)));
        }), []);
    }
    ...
    function topActions() {
        ...
        if (window.innerWidth < 991) menu.style.paddingTop = headerHeight + infoHeight + "px";
        const close = infoBlock.querySelector(".info__close");
        close.addEventListener("click", (() => {
            sessionStorage.setItem("infoBlockState", "hidden");
            header.style.height = headerHeight + "px";
            if (window.innerWidth < 991) menu.style.paddingTop = headerHeight + "px";
            infoBlock.classList.add("info--fade-out");
        ...
        menuLinks.forEach((link => {
            link.addEventListener("click", (e => {
                ...
                if (targetSection) {
                    ...
                    const offset = scrollingDown ? 1e3 : -400;
